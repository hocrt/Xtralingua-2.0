...
class CustomOptions extends Component {
  ...
  addScriptDialog(e) {
    ...
    dialog
      .showOpenDialog(this.props.electron.remote.getCurrentWindow(), {
        ...
        defaultPath: this.props.isDev
          ? '/home/panagiotis/Documents/gsoc2019-text-extraction/data'
  ...
  addScript(e) {
    ...
    this.props.setDistantState({
      selectedCustomScripts: [
        ...this.props.selectedCustomScripts,
        this.state.name,
      ],
    })
    this.handleClose()
  ...
  handleToggleAll() {
    if (
      this.props.selectedCustomScripts.length === this.props.savedScripts.length
    ) {
      this.props.setDistantState({ selectedCustomScripts: [] })
    } else {
      this.props.setDistantState({
        selectedCustomScripts: this.props.savedScripts.map(
        ...
        <Paper classes={{ root: classes.paper }}>
          <div className={classes.root}>
            <Checkbox
              ...
              checked={
                this.props.selectedCustomScripts.length ===
                  this.props.savedScripts.length &&
                this.props.selectedCustomScripts.length !== 0
              }
              indeterminate={
                this.props.selectedCustomScripts.length !==
                  this.props.savedScripts.length &&
              ...
              return this.props.savedScripts.map((scriptObj, index) => (
                <Accordion key={scriptObj.name}>
                  ...
                  >
                    <div>
                      <Checkbox
                        onClick={(e) => this.handleToggle(e, scriptObj.name)}
                        checked={
                          this.props.selectedCustomScripts.indexOf(
                            scriptObj.name
                          ) !== -1
                        }
                      />
                      <IconButton
                        onClick={(event) => {
                          ...
                          // }
                          let selectedCustomScripts = Object.assign(
                            [],
                            this.props.selectedCustomScripts
                          )
                          if (
                            selectedCustomScripts.indexOf(scriptObj.name) !== -1
                          ) {
                            selectedCustomScripts.splice(
                              selectedCustomScripts.indexOf(scriptObj.name),
                              1
                            )
                          }
                          this.props.setDistantState({
                            selectedCustomScripts: selectedCustomScripts,
                          })
                        }}
                        className={classes.button}
                      >
                      ...
                      <Typography display="inline" className={classes.heading}>
                        {scriptObj.name}
                      </Typography>
                    </div>
                  </AccordionSummary>
          ...
          <DialogContent>
            <form className={classes.container}>
              <FormControl
                ...
                error={
                  this.props.savedScripts
                    .map((scriptObj) => scriptObj.name)
              ...
              <FormControl required className={clsx(classes.formControl)}>
                <InputLabel htmlFor="script-environment">
                  Environment
                </InputLabel>
                <Select
                ...
                >
                  {this.state.environments.map((envObj, i) => (
                    <MenuItem key={i} value={envObj.path}>
                      {envObj.displayName}
                    </MenuItem>
                  ))}
                </Select>
