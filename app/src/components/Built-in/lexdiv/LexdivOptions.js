...
class LexdivOptions extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      scriptPath: 'src\\Built-in\\lexdiv\\lexdiv_indices.R',
    }
  }
  ...
  handleToggle(value) {
    ...
    if (value === 'all') {
      if (this.state.selectAll) {
        ...
        newChecked = this.state.lexdivIndices.map(
  ...
  changeArgs(e) {
    if (
      ...
      const args = [`${this.props.settings.get('rlibPath')}`]
        ...
        .concat(`-lexdivIndex=${this.props.selectedIndices.join(',')}`)
  ...
  render() {
    return (
      <div>
        ...
        <Button variant="contained" size="small" onClick={this.changeArgs}>
          add
        </Button>
      </div>
    )
