...
class ReadabilityOptions extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      readabilityIndices: [
        ...
        {
          indexName: 'Farr.Jenkins.Paterson',
          displayName: 'Farr.Jenkins.Paterson',
        },
        ...
        {
          indexName: 'FORCAST.RGL',
          displayName: 'FORCAST Reading Grade Level',
        },
        ...
        {
          indexName: 'meanSentenceLength',
          displayName: 'Mean Sentence Length',
        },
        { indexName: 'meanWordSyllables', displayName: 'Mean Word Syllables' },
      ],
      env: `${props.settings.get('rPath', '')}\\Rscript`,
      scriptPath: 'src\\Built-in\\readability\\readability_indices.R',
    }
  }
  ...
  handleToggle(value) {
    ...
    if (value === 'all') {
      if (this.state.selectAll) {
        ...
        newChecked = this.state.readabilityIndices.map(
          (indexObj) => indexObj.indexName
        )
  ...
  changeArgs(e) {
    if (
      ...
      const args = [`${this.props.settings.get('rlibPath')}`]
        .concat(`-filePaths=${this.props.filePaths.join(',')}`)
        .concat(`-readIndex=${this.props.selectedIndices.join(',')}`)
      this.props.setScriptParameters(
        false,
        this.props.type,
        this.state.env,
    ...
    return (
      <div>
        <Typography variant="subtitle1" align="center">
          Select one or more indices to extract
        </Typography>
        <GridList cols={5} cellHeight="auto">
          ...
          {this.state.readabilityIndices.map((indexObj, i) => (
            <ListItem
              key={i}
              button
              onClick={() => this.handleToggle(indexObj.indexName)}
            >
              <Checkbox
                checked={
                  this.state.selectAll ||
                  this.props.selectedIndices.indexOf(indexObj.indexName) !== -1
                }
              />
        ...
        <Button size="small" variant="contained" onClick={this.changeArgs}>
          add
        </Button>
      </div>
    )
