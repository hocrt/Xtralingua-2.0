...
class FilesTab extends Component {
  constructor(props) {
    ...
    this.addFilesDialog = this.addFilesDialog.bind(this)
      ...
      .then(async (retObj) => {
        if (retObj.filePaths !== undefined) {
          ...
          this.props.logMessage(
            `Dialog closed. ${
              insertResults.upsertedCount !== 0
                ? `${insertResults.upsertedCount} file(s) added.`
                : 'No files added.'
            } `,
            'info'
          )
  ...
  handleToggleAll() {
    if (this.props.selectedFilesPaths.length === this.props.files.length) {
      ...
      this.props.setDistantState({
        selectedFilesPaths: this.props.files.map((fileObj) => fileObj.path),
      })
    }
  }
                ...
                <TableRow>
                  <StyledTableCell>
                    <div className={classes.flexContainer}>
                      <Checkbox
                        checked={
                          this.props.selectedFilesPaths.length ===
                            this.props.files.length &&
                          this.props.selectedFilesPaths.length !== 0
                        }
                        indeterminate={
                          this.props.selectedFilesPaths.length !==
                            this.props.files.length &&
                  ...
                  {(() => {
                    try {
                      return Object.keys(this.props.files[0]).map((field) => {
                        const id = columnId++
                        return (
                  ...
                  >
                    <StyledTableCell padding="checkbox">
                      <div className={classes.flexContainer}>
                        <Checkbox
                          checked={
                            this.props.selectedFilesPaths.indexOf(
                              fileObj.path
                            ) !== -1
                          }
                        />
