...
class ResultsTab extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      tabIndex: 0,
      tabs: props.additionalResults.map((resultObj) =>
        this.generateWordListElement(resultObj.wordList)
      ),
    }
    this.resultTable = []
    ...
    this.exportResultsDialog = this.exportResultsDialog.bind(this)
    this.handleMenuToggle = this.handleMenuToggle.bind(this)
    this.handleMenuItemClick = this.handleMenuItemClick.bind(this)
    this.closeSnackbar = this.closeSnackbar.bind(this)
    this.handleClose = this.handleClose.bind(this)
    this.rowRenderer = this.rowRenderer.bind(this)
    this.generateWordListElement = this.generateWordListElement.bind(this)
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.resultList !== this.props.resultList) {
      ...
      this.props.setDistantState({
        selectedResultRows: [...Array(this.props.resultList.length).keys()],
      })
    }
  }
    ...
    resultList.forEach((bookObj) => {
      ...
      try {
        Object.keys(indices).map((index) => {
          if (indices[index].length !== 0) bookObj[index] = indices[index]
        })
      } catch (e) {
  ...
  exportResultsDialog() {
    ...
    exportArray = exportArray.concat(
      this.props.selectedResultRows
        .sort()
        .map((index) => this.resultTable[index + 1])
    )
    ...
    dialog
      .showSaveDialog(this.props.electron.remote.getCurrentWindow(), {
      ...
      .then((retObj) => {
        ...
        if (filePath !== undefined) {
          ...
          switch (type) {
            case 'csv':
              exportString = exportArray
                .map((exportList) => exportList.join(','))
                .join('\n')
              break
            case 'txt':
              exportString = exportArray
              ...
              break
            case 'json':
              exportTitles = exportArray[0]
              exportData = exportArray.slice(1).map((dataList) => {
                let tempObj = {}
                dataList.forEach(
                  (fieldValue, index) =>
                    (tempObj[exportTitles[index]] = fieldValue)
                )
                return tempObj
              })
  ...
  rowRenderer({ key, index, isScrolling, isVisible, style }, wordList) {
    return (
      <ListItem
        key={index}
        classes={{ root: this.props.classes.listRow }}
        style={style}
        dense
      >
        ...
        <AutoSizer>
          {({ height, width }) => (
            <List
              ...
              rowHeight={30}
              rowRenderer={(defaultParams) =>
                this.rowRenderer(defaultParams, wordList)
              }
            />
          )}
        </AutoSizer>
  ...
  getWordList(bookRow, type) {
    ...
    const index = this.props.additionalResults
      .filter((resultObj) => resultObj.wordListType === type)
      .map((resultObj) => resultObj.path)
      .indexOf(filePath)
    if (index === -1) {
      ...
      this.setState({
        tabs: [...this.state.tabs, this.generateWordListElement(wordList)],
      })
    }
  ...
  render() {
                  ...
                  <TableRow>
                    <TableCell>
                      <Checkbox
                        ...
                        checked={
                          this.props.selectedResultRows.length ===
                            this.props.resultList.length &&
                          this.props.selectedResultRows.length !== 0
                        }
                        indeterminate={
                          this.props.selectedResultRows.length !==
                            this.props.resultList.length &&
                    ...
                    {Object.keys(this.state.resultList[0]).map(
                      (indexName, i) => {
                        if (
                          typeof this.state.resultList[0][indexName] !==
                            typeof {} ||
                          Array.isArray(this.state.resultList[0][indexName])
                        ) {
                          const id = columnId++
                          ...
                          return (
                            <TableCell rowSpan="2" key={i}>
                              <TableSortLabel
                                ...
                                onClick={() =>
                                  this.sortByColumn(`${indexName}`, id)
                                }
                              >
                                {indexName}
                              </TableSortLabel>
                        ...
                        return Object.keys(
                          this.state.resultList[0][indexName]
                        ).map((el, idx) => {
                          const id = columnId++
                          ...
                          return (
                            <TableCell key={idx}>
                              <TableSortLabel
                                ...
                                onClick={() =>
                                  this.sortByColumn(`${indexName}.${el}`, id)
                                }
                              >
                                {el}
                              </TableSortLabel>
            ...
            {(() => {
              try {
                return this.state.resultList.map((elem, i) => {
                  let columnId = 0
                  return (
                    <TableRow key={i} onClick={() => this.handleToggle(i)}>
                      <TableCell>
                        <Checkbox
                          checked={
                            this.props.selectedResultRows.indexOf(i) !== -1
                          }
                        />
                      </TableCell>
                              ...
                              return (
                                <TableCell key={id}>
                                  <div
                                    className={classes.link}
                                    onClick={(e) =>
                                      this.showWordList(e, i, 'vocabulary')
                                    }
                                  >
                                    {value}
                                  </div>
    ...
    const snackbar = (
      ...
      >
        <SnackbarContent
          message={
            <Typography classes={{ root: classes.errorTitle }}>
              There were errors during execution. Showing stored results for
              selected files, which might be partial, outdated and/or wrong.
            </Typography>
          }
          action={[
            <IconButton
    ...
    return (
      <div className={classes.outerContainer}>
        ...
        <Tabs
          ...
          variant="scrollable"
          scrollButtons="auto"
          onChange={(event, tabIndex) => this.changeTab(tabIndex)}
        >
          <Tab label="Results" />
        ...
        <Container maxWidth="lg" classes={{ root: classes.container }}>
          ...
          <div className={classes.buttonContainer}>
            <Button
              ...
              className={clsx({ [classes.disabled]: this.props.processing })}
              onClick={this.props.executeAll}
            >
              Execute
            </Button>
            <ButtonGroup
            ...
            >
              <Button
                variant="contained"
                disabled={this.props.selectedResultRows.length === 0}
                className={clsx({
                  [classes.disabled]:
                    this.props.selectedResultRows.length === 0,
                })}
                onClick={this.exportResultsDialog}
              >
                Export selected as {this.state.selectedExportType.displayName}
              </Button>
              <Button
                color="primary"
                variant="contained"
                  ...
                  <MenuList>
                    {this.state.exportTypes.map((typeObj, index) => (
                      <MenuItem
                        key={index}
                        selected={
                          typeObj.type === this.state.selectedExportType
                        }
